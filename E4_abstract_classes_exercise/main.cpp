    ...
    switch (gameEvent) {
        ...
        case GameEvent::fight: {
            if (hero.isLegalFight(enemy)) {
                ...
                if (result && enemy.getHP()>0) {
                    std::cout << "Enemy hit ! (HP: " << enemy.getHP() << ")" << std::endl;
                } else if (!result &&  enemy.getHP()>0) {
                    std::cout << "Hit missed..." << std::endl;
                } else {
                    std::cout << "Enemy already death" << std::endl;
                }
            } else {
        ...
        case GameEvent::magic: {
             if (MageKnight* magenight = dynamic_cast<MageKnight*>(&hero)) {
                if (magenight ->isLegalFight(enemy)){
                    magenight->doMagic();
                }
            } else if ( Wizard* wizard = dynamic_cast<Wizard*>(&hero)) {
                wizard->doMagic();
            }
            break;
        }
...
void renderHUD(GameCharacter &hero) {
    ...
    if (typeid(hero) == typeid(Wizard) || typeid(hero) == typeid(MageKnight))
        std::cout << ",m"; // only Wizards and MageKnight can do magic
