...
bool updateGame(const GameEvent &gameEvent, GameCharacter &hero, GameCharacter &enemy, const Dungeon &map) {
    switch (gameEvent) {
        ...
        case GameEvent::fight: {
            ...
            if (hero.getPosX()==(enemy.getPosX()-1) || hero.getPosY() == (enemy.getPosY()-1) || hero.getPosX()==(enemy.getPosX()+1) || hero.getPosY() == (enemy.getPosY()+1)) {
                hero.fight(enemy);
            } else {
                std::cout << "the Fight can't be done" << std::endl;
            }
            break;
        }
        case GameEvent::noop: {
...
void renderHUD(GameCharacter &hero) {
    ...
    if (hero.getWeapon()){
        std::cout << "Weapon Strength: " << hero.getWeapon()->getStrength()<<std::endl;
    }
    std::cout << std::endl;
}
